...
public class PyrolysisRecipe implements Recipe<SimpleContainer> {
    ...
    public PyrolysisRecipe(ResourceLocation pId, Ingredient pRecipeItem, ItemStack pOutput1, ItemStack pOutput2) {
        this.id = pId;
    ...
    @Override
    public RecipeSerializer<?> getSerializer() { return ModRecipeSerializers.PYROLYSIS_SERIALIZER.get(); }

    @Override
    public RecipeType<?> getType() { return ModRecipeTypes.PYROLYSIS_TYPE.get(); }



    // --- 5. Serializerの内部クラス ---
    public static class Serializer implements RecipeSerializer<PyrolysisRecipe> {
        @Override
        public PyrolysisRecipe fromJson(ResourceLocation pRecipeId, JsonObject pSerializedRecipe) {
            ...
            return new PyrolysisRecipe(pRecipeId, input, output1, output2);
        }

        @Nullable
        @Override
        public PyrolysisRecipe fromNetwork(ResourceLocation pRecipeId, FriendlyByteBuf pBuffer) {
        ...
        @Override
        public void toNetwork(FriendlyByteBuf pBuffer, PyrolysisRecipe pRecipe) {
            pRecipe.getIngredients().get(0).toNetwork(pBuffer); // 材料リストの0番目を取得して書き込む
